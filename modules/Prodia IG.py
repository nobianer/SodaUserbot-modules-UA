...
class ProdiaMod(loader.Module):
    ...
    strings_ua = {
        "name": "Prodia IG",
        "model-h": "Початковий набір зображень, на основі якого побудовано модель.",
        "cfg-h": "Що вище значення, то ближчим до запиту буде зображення.",
        "sampler-h": "Алгоритм, за яким буде згенеровано зображення.",
        "steps-h": "Кількість кроків для генерації зображення, чим вище значення, "
                   "тим більш деталізованим буде зображення.",
        "ndef-h": "Стандартний негативний запит. Негативний промпт змушує ШІ НЕ генерувати "
                  "описану в ньому річ.",
        "args?": "🟡 <b>Ви не вказали аргументів…</b>",
        "working": "🎨 <b>Працюю над зображенням…</b>\n"
                   "<b>Запит:</b> <code>{}</code>{}\n\n"
                   "<b>Вказані параметри:</b>\n"
                   "<i>Модель:</i> {} — ID <code>{}</code>\n"
                   "<i>CFG</i>: {}\n"
                   "<i>Семплер:</i> {}\n"
                   "<i>Кроки:</i> {}",
        "done": "🎉 <b>Ваше зображення готове!</b>\n"
                "<b>Запит:</b> <code>{}</code>{}\n\n"
                "<i>Модель:</i> {} — ID <code>{}</code>\n"
                "<i>CFG:</i> {}\n"
                "<i>Семплер:</i> {}\n"
                "<i>Кроки:</i> {}\n",
        "neg_prompt": "\n<i>Негативний запит</i>: <code>{}</code>",
        "_cls_doc": "Генератор зображень на основі Prodia API. Не потребує API ключа.",
        "_cmd_doc_prodia": "Згенерувати зображення за допомогою Prodia API.",
    }
